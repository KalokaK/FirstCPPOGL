...
void main()
{
    ...
    float decayedLocal = 2 / ((distance(localOrigin, TexCoord) + 1) * 2); // intensity. 1/x but shifted and scaled
    vec3 lightColor = vec3(1, 0.95, 0.95); // dugh
    vec4 texCol = texture(text, TexCoord); // dugh
    float lightStrenght = (1 / ((distance(origin, pos.xy) + 1) * 30)) + decayedLocal; // same thing, just global
